        ...
        <div class="row">
            <div class="box box-primary">
                <div class="box-header">
                    <h3 class="box-title"><i class="fa fa-cog"></i> <spring:message code="calpoint.title" text="message not found"/></h3>
            </div>
            <div class="box-body">
                ...
                <table id="availableCalpointTable" class="dataTable hover cell-border nowrap">
                    <thead>
                        ...
                        <tr class="alt" >
                            <th><spring:message code="calpoint.calpointId" text="message not found"/></th>
                            <th><spring:message code="calpoint.calpointMin" text="message not found"/></th>
                            <th><spring:message code="calpoint.calpointMax"  text="message not found" /></th>
                            <th></th>
                            <th><button class="deleteMultiple btn btn-danger"><spring:message code="calpoint.delete" text="message not found"/><i class="fa fa-trash-o" aria-hidden="true"></i></button></th>
                        </tr> 
                    </thead>
                </table>
                <br/>
                <table id="unavailableCalpointTable" class="dataTable hover cell-border nowrap">
                    <thead>
                        ...
                        <tr class="alt" >
                            ...
                            <th><spring:message code="calpoint.calpointMin" text="message not found"/></th>
                            <th><spring:message code="calpoint.calpointMax"  text="message not found" /></th>
                            <th><button class="reuseMultiple btn btn-success"><spring:message code="calpoint.reuse" text="message not found"/><i class="fa fa-recycle" aria-hidden="true"></i></button></th>
                            <th><button class="realDeleteMultiple btn btn-danger"><spring:message code="calpoint.realDelete" text="message not found"/><i class="fa fa-remove" aria-hidden="true"></i></button></th>
                        </tr> 
                    </thead>
                </table>
            </div>
...
<script>
    ...
    function showFormByClick() {
        $("#ajaxCRUDfield").load($(this).attr("value"), function () {
            var $inputs = $('input[type=text][name=calpointMax],input[type=text][name=calpointMin]');
    ...
    function deleteMultiple() {
        var idArray = [];
        var size;
        $("input[type='checkbox'][name='deletedCalpoint']:checked").each(function (i) {
    ...
    function reuseMultiple() {
        var idArray = [];
        var size;
        $("input[type='checkbox'][name='reuseCalpoint']:checked").each(function (i) {
            idArray[i] = parseInt($(this).val());
        });
        size = idArray.length;
        if (size > 0) {
    ...
    function realDeleteMultiple() {
        var idArray = [];
        var size;
        $("input[type='checkbox'][name='realDeletedCalpoint']:checked").each(function (i) {
            idArray[i] = $(this).val();
        });
        size = idArray.length;
        if (size > 0) {
    ...
    function sendDataPOSTByAction() {
        ...
        var idfield = $("input[type=hidden][id=calpointId]");
        var id = "lastest";
    ...
    function validateInput() {
        valid = 1;
        var min = $("#calpointMin").val();
        var max = $("#calpointMax").val();
        var id = $("#calpointId").val();
        if ($("#calpointMax").val().trim().length <= 0 && $("#calpointMin").val().trim().length <= 0) {
            valid = 0;
            bootbox.alert({
                backdrop: true,
                className: "dangerFont",
                message: "please Enter calculation point",
                callback: function () { /* your callback code */
                }
            });
        }
        $.ajax({
            ...
            async: false,
            data: {min: min, max: max, id: id}, // serializes the form's elements.
            success: function (result)
            {
                if (result.length > 0) {
                    bootbox.alert({
